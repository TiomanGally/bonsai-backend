...
class BonsaiV1Controller(
    private val bonsaiManager: BonsaiManager,
    ...
    fun addBonsai(
        ...
        val userUuid = authentication.getUuidOfUser()
        ...
        return ResponseEntity.ok()
            .headers { it.add("x-bonsai-uuid", uuid.toString()) }
    ...
    fun getAllBonsais(
        authentication: Authentication
    ): ResponseEntity<List<BonsaiResource>> {
        return bonsaiManager
            .getAllBonsais(authentication.getUuidOfUser())
            .map { BonsaiResource.from(it) }
            .let { ResponseEntity.ok(it) }
    }

    @GetMapping("/{bonsai-uuid}")
    fun getBonsaiById(@PathVariable("bonsai-uuid") bonsaiUUID: UUID): BonsaiResource = bonsaiManager
        .getBonsai(bonsaiUUID)
        ?.let { BonsaiResource.from(it) }
        ?: throw BonsaiNotFoundException(bonsaiUUID)
    ...
    private fun Authentication.getUuidOfUser(): UUID {
        val email = (this as BonsaiUser).email
        return userService.findUserByEmail(email)?.uuid ?: throw UserNotFoundException(email)
    }
}
